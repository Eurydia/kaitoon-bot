...
from chemistry.solvers.solver_periodni import (
    _OxidationNumberAssignment,
    _SimpleBalance,
    _RedoxBalance,
)
from chemistry.solvers.solver_chemequations import _ReactionPrediction
...
class RedoxBalance(ChemistryCogs):
    ...
    async def single(
        self, ctx: Context, median: str, reactants: str, products: str
    ) -> None:
...
class ReactionPrediction(ChemistryCogs):
    ...
    async def single(
        self, ctx: Context, reactants: str = "", products: str = "", page: int = 1
    ) -> None:
...
class HelpCommand(ChemistryCogs):
    ...
    @staticmethod
    def _command_embed(
        cmd_name: str, aliases: Tuple[str], syntax: str, description: str
    ) -> Embed:
    ...
    async def bake_a_cake(self, ctx: Context) -> None:
        ...
        embed = Embed(
            title="Balanced 🍰 of 🍰 Reaction(s).", color=Colour.from_rgb(255, 119, 188)
        )
        embed.add_field(
