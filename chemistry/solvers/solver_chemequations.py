...
class _ReactionPrediction(ChemEquation):
    def solve(
        self, reactants: str = "", products: str = "", page_num: int = 1
    ) -> Embed:
        ...
        page_num -= 1
        p_react = self._parse_input(reactants)
        p_prod = self._parse_input(products)
        query = self._prepare_query(p_react, p_prod)
        page_source = self._prepare_page_source(
            "https://chemequations.com/en/advanced-search/?", f"{query}&submit="
        )
    ...
    @staticmethod
    def _parse_input(string: str) -> str:
        string = string.replace(" + ", " ")
        string = string.replace(",", " ")
        string = " + ".join(map(lambda s: s.strip(), string.split()))
        return string
    ...
    def _format_page_source(
        self, page_source: str, page_num: int
    ) -> Union[Tuple, None]:
    ...
    def _prepare_embed(
        ...
        embed.add_field(name="Interpreted as", value=interpreted_as, inline=False)
