...
class Periodni(ChemistrySolverBaseClass):
    ...
    def _prepare_query(queries: Tuple[str]) -> str:
        ...
        for query in queries:
            ...
            query_r = "%2B".join(map(lambda r: r.strip(), query_r.split()))
            query_p = "%2B".join(map(lambda p: p.strip(), query_p.split()))
            res.append(f"{query_r}%3D{query_p}")
        return tuple(res)

    @staticmethod
    def _prepare_page_source(base_url: str, queries: Tuple[str]) -> Tuple[str]:
    ...
    def _embed_info(
        ...
        if succeed_percentage < 50:
            ...
            embed.add_field(
                name="Status", value=f"❎ Failed ({succeed_percentage}%)", inline=False
            )
        else:
            embed.color = Colour.green()
            embed.add_field(
                name="Status", value=f"✅ Success ({succeed_percentage}%)", inline=False
            )
        embed.add_field(
            name="Inputs",
...
class _OxidationNumberAssignment(Periodni):
    def solve(self, compound: str, *args) -> List[Embed]:
        ...
        page_sources = self._prepare_page_source(
            "https://www.periodni.com/oxidation_numbers_calculator.php?eq=", c_urls
        )
    ...
    def _format_page_source(
        ...
        interpreted_as = soup.find("input", {"name": "equation", "class": "txt-c"}).get(
            "value"
        )
    ...
    def _prepare_embed(
        ...
        return self._embed_info(
            embed, succeed_percentage, compounds, interpreted_as, time_elapsed
        )
...
class _SimpleBalance(Periodni):
    def solve(self, reactions: Tuple[str]) -> Embed:
        ...
        page_sources = self._prepare_page_source(
            "https://www.periodni.com/balancing_chemical_equations.php?eq=", r_urls
        )
        interpreted = []
        unicodes = []
        results = []
    ...
    def _prepare_embed(
        ...
        succeed_percentage = round(succeed / total_coumpound, 2) * 100
        return self._embed_info(
            embed, succeed_percentage, reactions, interpreted_as, time_elasped
        )
...
class _RedoxBalance(Periodni):
    def solve(self, reactions_with_median: Tuple[str, str]) -> Embed:
        if any(
            median.lower() not in {"a", "acidic", "b", "basic"}
        ...
        page_sources = self._prepare_page_source(
            "https://www.periodni.com/ars_method.php?eq=", queries
        )
        interpreted = []
        unicodes = []
        results = []
    ...
    def _format_page_source(
        ...
        error_blocks = soup.findAll(attrs={"class": "crven"})
        if not result_div or sum(1 for e in error_blocks if e.name != "span"):
            return interpreted_as, None, None
        unicode = self._prepare_result_unicode(result_div)
        normal = self._prepare_result(unicode)
        return interpreted_as, unicode, normal
    ...
    def _prepare_embed(
        ...
        succeed = sum(1 for res in results if res)
        embed = Embed(
            title=f"Balanced {succeed} of {total_coumpound} Redox Reaction(s)."
        )
